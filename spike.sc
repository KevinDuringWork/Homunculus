...
val output = new java.io.File("spike-in.vcf.gz")
...
// Read Assembly
val GRCh37 = new IndexedFastaSequenceFile(
        new File("GRCh37.p13.genome.fa"),
        new FastaSequenceIndex(
            new File("GRCh37.p13.genome.fa.fai")
        )
    )

// Read NA12878 
val vcf_reader = AbstractFeatureReader.getFeatureReader(
    (new File(s"NA12878.vcf.gz")).getAbsolutePath(), 
    new VCFCodec(), true)
